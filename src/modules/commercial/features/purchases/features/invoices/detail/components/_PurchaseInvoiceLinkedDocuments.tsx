...
export function _PurchaseInvoiceLinkedDocuments({ invoice }: Props) {
  const isNC = isCreditNote(invoice.voucherType);

  const totalPayments = invoice.paymentOrderItems.reduce((sum, item) => sum + item.amount, 0);
  ...
  const totalCNLinkedRaw = !isNC
    ...
    : 0;
  // Capear fallback para que pagado no supere el total
  const maxFallbackCN = Math.max(0, invoice.total - totalPayments - totalCNAppliedExplicit);
  ...
  const hasLinkedDocuments =
    ...
    invoice.paymentOrderItems.length > 0 ||
      ...
      <CardContent className="space-y-6">
        ...
        {invoice.originalInvoice && (
          <div>
            ...
            <Link
              href={`/dashboard/commercial/purchases/${invoice.originalInvoice.id}`}
              ...
              {invoice.creditDebitNotes.map((doc) => (
                <div key={doc.id} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {PURCHASE_INVOICE_STATUS_LABELS[doc.status as PurchaseInvoiceStatus]}
                    </Badge>
        ...
        {/* Pagos (órdenes de pago) - solo para facturas/ND */}
        {!isNC && invoice.paymentOrderItems.length > 0 && (
          <div>
            <h4 className="text-sm font-medium text-muted-foreground mb-2">Pagos</h4>
            <div className="space-y-2">
              {invoice.paymentOrderItems.map((item, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    <Receipt className="h-4 w-4 text-muted-foreground" />
                    <span className="font-mono">{item.paymentOrder.fullNumber}</span>
                    <span className="text-muted-foreground">
                      {moment(item.paymentOrder.date).format('DD/MM/YYYY')}
                    </span>
                    <Badge
                      variant={item.paymentOrder.status === 'CONFIRMED' ? 'default' : 'outline'}
                      className={item.paymentOrder.status === 'CONFIRMED' ? 'bg-green-600' : ''}
                    >
                      {item.paymentOrder.status === 'CONFIRMED' ? 'Confirmada' : 'Borrador'}
                    </Badge>
                  </div>
              ...
              {invoice.creditNoteApplicationsReceived.map((app, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    ...
                    <Link
                      href={`/dashboard/commercial/purchases/${app.creditNote.id}`}
                      className="text-primary hover:underline font-mono"
                    >
              ...
              {invoice.creditNoteApplicationsGiven.map((app, idx) => (
                <div key={idx} className="flex items-center justify-between text-sm">
                  <div className="flex items-center gap-2">
                    <FileText className="h-4 w-4 text-muted-foreground" />
                    <Link
                      href={`/dashboard/commercial/purchases/${app.invoice.id}`}
                      className="text-primary hover:underline font-mono"
                    >
        ...
        {invoice.status !== 'DRAFT' && (
          <>
            ...
            <div className="space-y-2">
              ...
              {!isNC && totalPayments > 0 && (
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Pagado (órdenes de pago):</span>
              ...
              <div className="flex justify-between font-semibold">
                ...
                <span className="font-mono">
                  ...
                  {isNC
                    ...
                    : (invoice.total - totalPayments - totalCNApplied).toLocaleString('es-AR', {
                        minimumFractionDigits: 2,
                      })}
                </span>
              </div>
